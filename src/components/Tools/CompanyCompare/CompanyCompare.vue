...
<template>
  <div class="flex flex-col mt-3 flex-1">
    ...
    <div class="companies-container">
      <div class="mode-banner" :class="{ 'mode-banner--cloud': isCloudMode }">
        <div class="mode-copy">
          ...
          <span>
            {{ isCloudMode ? '公司记录已保存到账号，可跨设备使用。' : '无需登录即可使用，记录只保存在当前浏览器；清除浏览器数据、更换浏览器或设备后，本地记录可能丢失；登录后可长期保存并在多个设备之间同步。' }}
          </span>
        </div>
    ...
    <ToolDetail title="描述">
      <el-text>
        公司对比工具，帮助你记录和对比不同公司的待遇信息。未登录时数据保存在当前浏览器本地，清除浏览器数据、更换浏览器或设备后可能丢失；登录后可保存到云端并跨设备使用。支持录入薪资、工作时间、福利待遇、加班情况等多维度信息，让你更好地评估和选择心仪的工作机会。
      </el-text>
    </ToolDetail>
  </div>
</template>
