...
const info = reactive({
  title: "待办事项",
  desc: "在线待办事项管理工具，帮助您高效管理日常任务。支持设置优先级（低/中/高）、截止日期时间（精确到秒）、自定义分类，一键标记完成状态。未登录时数据保存在当前浏览器本地，清除浏览器数据、更换浏览器或设备后可能丢失；登录后可保存到云端并跨设备使用。"
})
  ...
  <div class="flex flex-col flex-1 bg-white rounded-md p-4 c-sm:p-6 mt-3">
    <div class="mode-banner" :class="{ 'mode-banner--cloud': isCloudMode }">
      <div class="mode-copy">
        ...
        <span>
          {{ isCloudMode ? '待办已保存到账号，可跨设备使用。' : '无需登录即可管理待办，数据只保存在当前浏览器；清除浏览器数据、更换浏览器或设备后，本地数据可能丢失；登录后可长期保存并在多个设备之间同步。' }}
        </span>
      </div>
