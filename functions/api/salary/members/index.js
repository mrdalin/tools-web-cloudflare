...
export async function onRequest(context) {
  ...
  try {
    ...
    const path = url.pathname.replace('/api/salary/members', '')
    const id = path.replace(/^\//, '')
    const hasId = id && id !== ''

    switch (request.method) {
      case 'GET':
        if (!hasId) return await controller.getMembers(user, origin)
        return await controller.getMember(id, user, origin)

      case 'POST':
        if (hasId) return ApiResponse.error('创建成员不需要提供ID', origin, 400)
    ...
    console.error('Salary Members API error:', error)
