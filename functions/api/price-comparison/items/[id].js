  ...
  try {
    ...
    switch (request.method) {
      ...
      case 'PUT': {
        ...
        return await controller.updateItem(id, updateData, user, origin)
      }

      case 'DELETE':
        return await controller.deleteItem(id, user, origin)

      default:
        return ApiResponse.error('不支持的请求方法', origin, 405)
    }
  } catch (error) {
    console.error('Price Comparison Item [id] API error:', error)
