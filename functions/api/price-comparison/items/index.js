...
export async function onRequest(context) {
  ...
  try {
    ...
    switch (request.method) {
      case 'GET':
        return await controller.getItems(user, origin, queryParams)

      case 'POST': {
        const createData = await request.json()
        return await controller.createItem(createData, user, origin)
      }
    ...
    console.error('Price Comparison Items API error:', error)
